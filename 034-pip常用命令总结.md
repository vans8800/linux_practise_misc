...
可以看到 Requires: 和 Required-by: 字段。
```
...
方案 1：直接使用 PyPI 官网搜索（最可靠）
访问：https://pypi.org/search/?q=numpy
...
# 使用 curl 查询 PyPI 的 JSON API
curl -s https://pypi.org/pypi/numpy/json | python -m json.tool | less

# 只看版本信息
curl -s https://pypi.org/pypi/numpy/json | python -c "import sys, json; data=json.load(sys.stdin); print('\n'.join(data['releases'].keys()))"
```
